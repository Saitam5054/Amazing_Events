...
const maxAttendanceEvent = events.reduce((max, event) => {
...
const minAttendanceEvent = events.reduce((min, event) => {
  const attendancePercent = (event.assistance / event.capacity) * 100;
...
const maxCapacityEvent = events.reduce((max, event) => {
...
tr1.innerHTML = `<td>${maxAttendanceEvent.name} (${maxAttendanceEvent.attendancePercent.toFixed(2)}%)</td>
                    <td>${minAttendanceEvent.name} (${minAttendanceEvent.attendancePercent.toFixed(2)}%)</td>
